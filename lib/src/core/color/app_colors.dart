import 'package:flutter/material.dart';

class AppColors {
  //light theme colors
  static const Color primaryColor = Color(0xFFF1FFF3);
  static const Color primarySecondaryColor = Color(0xFF35C55F);
  static const Color lightBackgroundColor = Color(0xFF35C55F);
  static const Color lightTextColor = Color(0xFF0E3E3E);
  static const Color lightGreyColor = Color(0xFFDFF7E2);

  //dark theme colors
  static const Color darkPrimaryColor = Color(0xFF093030);
  static const Color darkSecondaryColor = Color(0xFF35C55F);
  static const Color darkBackgroundColor = Color(0xFF093030);
  static const Color darkTextColor = Color(0xFFF1FFF3);
}
